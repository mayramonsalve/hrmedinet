...
(function($){
	$.widget("ui.formwizard", {
		...
		_show : function(step){
	        ...
	        if ($.inArray(step, this.activatedSteps) !== -1) { //MODIFICAR ESTA CONDICION
					backwards = true;
					this.activatedSteps.pop();
				}else {
					this.activatedSteps.push(step);
	  ...
	  /*Methods*/
		...
		options: {
			...
			back : ":reset",
			next : ":submit",
			textSubmit: 'Submit',// $("#ButtonSubmit").val(),
			textNext: 'Next',//$("#ButtonNext").val(),
			textBack: 'Back',//$("#ButtonBack").val(),
